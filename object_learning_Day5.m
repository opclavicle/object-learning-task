...
%% Day 5
% a preview
% 5 stimuli in control; 5 stimuli in ex peach conditon 2 times; total 400 

%% uncomment these parameters and comment 'function'-line if you want to run it like a script
% 
% subjID=102;
%  acq=2;   % output name 1 and 2
%  condition =[1 2];
...
fid = fopen([num2str(subjID) '_object_Day5_' num2str(acq) '.mat']);
if fid~=-1
...
for theBlock = 1:numBlocks
    ...
    for i = 1: n_staircasePerBlock % n_mask
        ...
        thepic=tempic+(day5_con-1)*n_stimuli;
        end
...
%                    this time is for fixation
     ...
     %   while responded < 3
      ...
      %  if responseChar(1)==correctResponse{thepic}(1) & responseChar(2)==correctResponse{thepic}(2) & responseChar(3)==correctResponse{thepic}(3)            
    ...
    Screen('CloseAll');
     ...
     save([num2str(subjID) '_object_Day5_' num2str(acq) '.mat'], 'key', 'theorder', 'theorderMasks', 'subjID', 'acq', 'frameRate', ...
